...
var models = [
        {label:"차량번호",			name:"carregno",    index:"carregno",	width: "75", 	align:"center", frozen:true},
        {label:"버스ID",		    name:"busid",       index:"busid",	    width: "50", 	align:"center", frozen:true},
        {label:"운수회사",			name:"compnm", 	    index:"compnm",     width: "120", 	align:"left",   frozen:true},
        {label:"배정노선",			name:"routeno",     index:"routeno",    width: "200", 	align:"left",   frozen:true},
        {label:"평균",			name:"dtavg", 	    index:"dtavg",	    width: "55", 	align:"center", frozen:true},
...
function initGrid(){
	...
	makeGrid("#car_list", models, 110, "resultList", null, onComplete, null);
	
	$("#car_list").jqGrid("setFrozenColumns");
	$(".frozen-bdiv").css("background-color","rgb(255,255,255)");
	
	function onComplete(data){
		hideLoader();
		$("#search_count").text("차량별 수집검색 결과  "+data.records+"건");
	}
	...
	for(var i=1; i<32; i++)
		HideCol(i);
	$("#min_lag_check").prop('disabled', true);
	$("#min_lag").prop('disabled',true);
}
function loadGrid(option){
	showLoader();
	var event_type;
	var time="0";
	if($("#min_lag_check").is(":checked")){
		if($("#min_lag").val()!="")
			time=$("#min_lag").val();	
	}
	...
	console.log(params)
	reloadGrid("#car_list", "./stop/selectBusEvent.do", params, "resultList");
}
function ShowCol(target){
	if(target<10)
		$('#car_list').showCol('dt'+'0'+target);
	else
		$('#car_list').showCol('dt'+target);
}
function HideCol(target){
	if(target<10)
		$('#car_list').hideCol('dt'+'0'+target);
	else
		$('#car_list').hideCol('dt'+target);
}
function SetLabel(target,content){
	if(target<10){
		$("#car_list").jqGrid('setLabel',"dt"+'0'+target, content);
	}
	else
		$("#car_list").jqGrid('setLabel',"dt"+target, content);
}
...
function SetDt_day(end,start){
	...
	while(endtime>=starttime && target<32){
		ShowCol(target);
...
function initEvent(){
	...
	$("#search_option").change(function(){
		if($(this).val()==1){
			...
			$(".ex_706").text("최대 24시간 조회 >");	
		}
		else{
			$(".hour_input").val("");
			$(".hour_input").attr('disabled','disabled');
			$(".ex_706").text("최대 1달 조회 >");
		}
	...
	$("#event_type").change(function(){
		if($(this).val()==5 && $("#search_option").val()==2){
			$("#min_lag_check").prop('disabled', false);
			$("#min_lag").prop('disabled', false);
		}
		else{
			$("#min_lag_check").prop('disabled', true);
			$("#min_lag_check").prop('checked', false)
			$("#min_lag").prop('disabled', true);
		}
	...
	$("#btn_excel").click(function(){
		if( 0 < $("#car_list").getGridParam("reccount") )
			execlDownload($(".pop_title2 h2").text(), "#car_list");
		else
