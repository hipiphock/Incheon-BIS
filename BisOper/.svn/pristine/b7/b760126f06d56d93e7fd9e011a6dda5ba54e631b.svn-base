...
public class BodyDownload {
	...
	private byte FILE_TYPE = 0x02; //1 뉴스 2 어플리케이션
...
//	private int SEND_TIME;                               //전송 시각	전송 시각	hhmmss	4	N					
	...
	public BodyDownload(int nCount) {
		cHeader = new CenterHeader();
		cTail = new CenterTail();
		mHeader = new Header();
		mTail = new Tail();		
		
		cHeader.setOPCODE((byte)0x90);
		mHeader.setSR_CNT((short) nCount);
		mHeader.setOPCODE((byte)0x90);
	}
